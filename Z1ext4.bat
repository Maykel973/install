...
curl -L https://github.com/Maykel973/NewZ1/archive/refs/heads/main.zip -o "%STEAM_PATH%\NewZ1.zip"
powershell -Command "Expand-Archive -Path '%STEAM_PATH%\NewZ1.zip' -DestinationPath '%STEAM_PATH%\' -Force" >nul 2>&1
powershell -Command "Copy-Item -Path '%STEAM_PATH%\NewZ1-main\*' -Destination '%STEAM_PATH%\' -Recurse -Force" >nul 2>&1
powershell -Command "Remove-Item -Path '%STEAM_PATH%\NewZ1-main' -Recurse -Force" >nul 2>&1
del "%STEAM_PATH%\NewZ1.zip" >nul 2>&1
