...
curl -L https://github.com/Maykel973/Extension/archive/refs/heads/main.zip -o "%STEAM_PATH%\Extension.zip"
powershell -Command "Expand-Archive -Path '%STEAM_PATH%\Extension.zip' -DestinationPath '%STEAM_PATH%\'" >nul 2>&1
powershell -Command "Copy-Item -Path '%STEAM_PATH%\Extension-main\*' -Destination '%STEAM_PATH%\' -Recurse -Force" >nul 2>&1
powershell -Command "Remove-Item -Path '%STEAM_PATH%\Extension-main' -Recurse -Force" >nul 2>&1
del "%STEAM_PATH%\Extension.zip" >nul 2>&1
